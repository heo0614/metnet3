...
class MetNet3(Module):
    ...
    def forward(
        ...
        # Surface logits 계산 및 dict 변환
        surface_logits = self.to_surface_bins(x).split(self.surface_bin_dims, dim=1)
        surface_preds = dict(zip(self.surface_bin_names, surface_logits))

        # Precipitation logits 계산 및 dict 변환
        precipitation_logits = self.to_precipitation_bins(x).split(self.precipitation_bin_dims, dim=1)
        precipitation_preds = dict(zip(self.precipitation_bin_names, precipitation_logits))

        # 예측 객체 생성
        pred = Predictions(surface_preds, hrrr_pred, precipitation_preds)

        if not any([exists(surface_targets), exists(hrrr_target), exists(precipitation_targets)]):
            return pred

        assert all([exists(surface_targets), exists(hrrr_target), exists(precipitation_targets)]), 'All targets must be provided for loss calculation'

        loss_breakdown = LossBreakdown(
            surface_loss_breakdown,
            hrrr_loss,
            precipitation_loss_breakdown,
            surface_logits,         # surface_logits 추가
            precipitation_logits    # precipitation_logits 추가
            )    
